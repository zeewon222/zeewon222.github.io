...
import { fontControls, fontSizeOptions } from "./lib/constants"
import { useUniqueClassName } from "./lib/useUniqueClassName"
import { colorFromToken } from "./lib/colorFromToken"
import { getVariantControls } from "./lib/variantUtils"
...
export function Select(props) {
    ...
    const className = useUniqueClassName("select", [defaultBackgroundColor, defaultBorderColor, defaultBorderWidth])
    ...
    const variants = {
        default: {
            backgroundColor: Color(colorFromToken(defaultBackgroundColor)).toValue(),
            caretColor: Color(colorFromToken(defaultTextColor)).toValue(),
            color: Color(colorFromToken(defaultTextColor)).toValue(),
            boxShadow: `inset 0 0 0 ${defaultBorderWidth}px ${Color(colorFromToken(defaultBorderColor)).toValue()}`,
        },
        disabled: {
            backgroundColor: Color(colorFromToken(disabledBackgroundColor)).toValue(),
            color: Color(colorFromToken(disabledTextColor)).toValue(),
            caretColor: Color(colorFromToken(disabledTextColor)).toValue(),
            boxShadow: `inset 0 0 0 ${disabledBorderWidth}px ${Color(colorFromToken(disabledBorderColor)).toValue()}`,
            opacity: disabledOpacity / 100,
        },
        focus: {
            backgroundColor: Color(colorFromToken(focusBackgroundColor)).toValue(),
            color: Color(colorFromToken(focusTextColor)).toValue(),
            caretColor: Color(colorFromToken(focusTextColor)).toValue(),
            boxShadow: `inset 0 0 0 ${focusBorderWidth}px ${Color(colorFromToken(focusBorderColor)).toValue()}`,
        },
    }
